...
(function () {
  ...
  function importRun(p) {
    ...
    if (target === 'wbs') ok = insertWbs(toInsert, projectId, batchId);
      ...
      var replaced = 0;
      if (p.overwrite && BIZ_KEYS[target]) {
        toInsert.forEach(function (rec) {
          var keys = BIZ_KEYS[target], vals = [], conds = [];
          keys.forEach(function (k) {
            var v = rec[k];
            if (v === null || v === undefined || (typeof v === 'string' && String(v).trim() === '')) return;
            vals.push(v); conds.push(k + '=?');
          });
          if (vals.length) {
            DB.exec('DELETE FROM ' + target + ' WHERE ' + conds.join(' AND '), vals);
      ...
      if (replaced) p._replaced = replaced;
    }
    ...
    return { ok: true, inserted: ok, skipped: skipped, replaced: replaced || 0, errors: errors.slice(0, 30), batch_id: batchId, total_rows: dataRows.length };
  }
  ...
  // 覆盖导入的业务主键：勾选“覆盖导入”时，按这些键匹配并替换已存在的旧行
  var BIZ_KEYS = {
    'output_rec': ['biz_date', 'wbs_name', 'item_name', 'unit'],
    'material_rec': ['biz_date', 'wbs_name', 'material_name', 'spec'],
    'labor_rec': ['biz_date', 'wbs_name', 'team_name', 'trade'],
    'cost_rec': ['biz_date', 'wbs_name', 'cost_type', 'subject'],
    'progress_rec': ['biz_date', 'wbs_name', 'item_name'],
    'measure': ['biz_date', 'wbs_name', 'content']
  };

...
/* ---------- 部位与计划节点：分级树形菜单 ---------- */
/* 按 项目→单项工程→单位工程→分部工程→分项工程 五级展开，点击节点下钻，逐层编辑。*/
const WBS_TREE = {
  LEVELS: ['项目', '单项工程', '单位工程', '分部工程', '分项工程'],
    ...
    box.innerHTML = `
      <div class="card">
        ...
        <div class="note" style="margin-bottom:12px">
          按 <b>项目 → 单项工程 → 单位工程 → 分部工程 → 分项工程</b> 五级划分（铁路工程标准层级）。
          最顶层为<b>项目</b>（如「通苏嘉甬ⅩⅠ标」），其下挂<b>单项工程</b>（如「桥涵工程」）、
          <b>单位工程</b>（如「XX桥」），再下钻分部、分项。点击节点下钻展开下级；
          每个节点可「＋下级 / 编辑 / 删除」。<b>上级部位</b>用下拉框关联，产值、材料、进度统计时会自动含入下级。
          层级（level）随上级自动推算，无需手填；徽章颜色按层级区分。
        </div>

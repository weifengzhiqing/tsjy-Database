...
App.reg({
  ...
  async render(el) {
   try {
    ...
    // 专业归类：将工点归入顶层专业（隧道工程 / 桥梁工程 / 路基 / 站改 / 其他附属）
    // 隧道（含正洞）单列「隧道工程」；桥涵/跨线桥归「桥梁工程」；路基及清表归路基；站场及站改配套归站改；其余进“其他附属”
    function categoryOf(name) {
      if (!name) return '其他附属';
      if (name.indexOf('隧道') >= 0) return '隧道工程';
      if (name === '正洞') return '隧道工程';
    ...
    el.innerHTML = `
      ...
      <div class="card"><h3>形象进度 · 按部位层级钻取（点击 ▸ 展开）</h3>
        ...
        <div class="dim" style="margin-top:10px">顶层按专业分组（隧道工程 / 桥梁工程 / 路基 / 站改 / 其他附属），其下为工点（如 五磊山隧道、慈城特大桥），再下为子部位（进口/出口/斜井）与形象进度项。开累完成量 = 优先取系统记录的累计完成量（如金山同步的"开累完成数量"），无累计时按完成量累加。每日同步为"最新快照覆盖"，看板只显示最新开累，不重复累加。</div>
      </div>`;
   } catch (e) {
